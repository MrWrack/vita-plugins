...
static void github_mark(float x,float y,float sc){ draw_icon(ICO_GITHUB,x,y,sc*.32f); }
static void crown_mark(float x,float y,float sc){ draw_icon(ICO_CROWN,x,y,sc*.34f); }
static void brand(float x,float y,float scale){
 github_mark(x,y-29*scale,scale);
 txt_dim(x+47*scale,y-12*scale,.70f*scale,"MADE BY");
 txt(x+47*scale,y+13*scale,.98f*scale,"MrWrack");
 crown_mark(x+183*scale,y-27*scale,.86f*scale);
}
